...
def xcorr_slow(x, kernel, padding=0):
    ...
    for i in range(batch):
        ...
        po = F.conv2d(px, pk, padding=padding)
        out.append(po)
    out = torch.cat(out, 0)
    return out


def xcorr_fast(x, kernel, padding=0):
    """group conv2d to calculate cross correlation, fast version
    """
    batch = kernel.size()[0]
    pk = kernel.view(-1, x.size()[1], kernel.size()[2], kernel.size()[3])
    px = x.view(1, -1, x.size()[2], x.size()[3])
    po = F.conv2d(px, pk, groups=batch, padding=padding)
    po = po.view(batch, -1, po.size()[2], po.size()[3])
    return po


def xcorr_depthwise(x, kernel, padding=0):
